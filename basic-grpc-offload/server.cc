...
class GreeterServiceImpl final : public Greeter::Service {
    ...
    Status DoFib(ServerContext* context, const FibIn* para_in, FibOut* para_out) override {
        struct timespec start, finish;
        uint32_t res;

        clock_gettime(CLOCK_REALTIME, &start);
        res =  fib(para_in->fib_count());
        ...
        printf("Recv! Result: %d, Elased time: [%.3f ms]\n", res, fms);    
        para_out->set_fib_back(res);
        return Status::OK;
    }
};
